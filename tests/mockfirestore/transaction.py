...
from mockfirestore._helpers import Timestamp, generate_random_string
...
class Transaction:
    ...
    def get(self, ref_or_query) -> Iterable[DocumentSnapshot]:
        if isinstance(ref_or_query, DocumentReference):
            ...
            raise ValueError('Value for argument "ref_or_query" must be a DocumentReference or a Query.')
...
class AsyncTransaction:
    ...
    async def get_all(self, references: Iterable[DocumentReference]) -> Iterable[DocumentSnapshot]:
        return await self._client.get_all(references)

    async def get(self, ref_or_query) -> Iterable[DocumentSnapshot]:
        if isinstance(ref_or_query, DocumentReference):
            return await self._client.get_all([ref_or_query])
        elif isinstance(ref_or_query, Query):
            return ref_or_query.stream()
        else:
            raise ValueError('Value for argument "ref_or_query" must be a DocumentReference or a Query.')
    ...
    async def _add_write_op(self, write_op: Callable):
        if self._read_only:
            raise ValueError("Cannot perform write operation in read-only transaction.")
        self._write_ops.append(write_op)

    async def create(self, reference: DocumentReference, document_data):
        # this is a no-op, because if we have a DocumentReference
        # it's already in the MockFirestore
        ...

    async def set(self, reference: DocumentReference, document_data: dict, merge=False):
        write_op = partial(reference.set, document_data, merge=merge)
        self._add_write_op(write_op)

    async def update(self, reference: DocumentReference, field_updates: dict, option=None):
        write_op = partial(reference.update, field_updates)
        self._add_write_op(write_op)

    async def delete(self, reference: DocumentReference, option=None):
        write_op = reference.delete
        self._add_write_op(write_op)
