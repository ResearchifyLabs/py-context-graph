...
from mockfirestore._helpers import Document, Store, Timestamp, delete_by_path, get_by_path, set_by_path
...
class AsyncDocumentSnapshot:
    def __init__(self, reference: 'AsyncDocumentReference', data: Document) -> None:
        self.reference = reference
        self._doc = deepcopy(data)
...
class AsyncDocumentReference:
    def __init__(self, data: Store, path: List[str], parent: 'AsyncCollectionReference') -> None:
        self._data = data
        self._path = path
        self.parent = parent
    ...
    async def get(self) -> AsyncDocumentSnapshot:
        return AsyncDocumentSnapshot(self, get_by_path(self._data, self._path))

    async def delete(self):
        delete_by_path(self._data, self._path)

    async def set(self, data: Dict, merge=False):
        if merge:
            try:
    ...
    def collection(self, name) -> 'AsyncCollectionReference':
        from mockfirestore.collection import AsyncCollectionReference
